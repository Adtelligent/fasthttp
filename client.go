...
func (c *Client) GetDeadlineWithRequest(dst []byte, req *Request, url string, deadline time.Time) (statusCode int, body []byte, headers string, err error) {
	return clientGetURLDeadlineWithRequest(dst, req, url, deadline, c)
}

func clientGetURLDeadlineWithRequest(dst []byte, req *Request, url string, deadline time.Time, c clientDoer) (statusCode int, body []byte, headers string, err error) {
	timeout := -time.Since(deadline)
	if timeout <= 0 {
		return 0, dst, "", ErrTimeout
	}
	...
	// usually continue execution on the host.
	go func() {
		statusCodeCopy, bodyCopy, headers, errCopy := doRequestFollowRedirects(reqCopy, dst, url, c)
		ch <- clientURLResponse{
			statusCode: statusCodeCopy,
			body:       bodyCopy,
			err:        errCopy,
